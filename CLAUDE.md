...
- **`extract`** (`src/vibe_resume/core/runner.py::run_extractors`) — drives all extractors via
  `ThreadPoolExecutor(max_workers=4)` with a `rich.progress` display. Each
  extractor returns `list[Activity]` (schema in `src/vibe_resume/core/schema.py`) which is
...
- **`aggregate`** (`src/vibe_resume/core/aggregator.py`) — groups activities by project and
...
- **`enrich`** (`src/vibe_resume/core/enricher.py`) — feeds each group through a locale-shaped
...
- **`render`** (`src/vibe_resume/render/renderer.py`) — merges `profile.yaml` + enriched
  groups into a Jinja2 template `src/vibe_resume/render/templates/resume.<locale>.md.j2`,
  ...
  DOCX for `ja_JP` uses a specialized path (`src/vibe_resume/render/japan.py`) that emits the
  JIS Z 8303 履歴書 grid form rather than going through pandoc.

- **`review`** (`src/vibe_resume/core/review.py`) — scores a rendered resume against the
...
Locales are registered in `src/vibe_resume/render/i18n.py::LOCALES`. Adding one requires:
the registry entry, a new `src/vibe_resume/render/templates/resume.<locale>.md.j2`, and —
if the review pitfalls (e.g. contact-line width) apply — a template tweak.

## Key contracts

- **`Activity` schema** (`src/vibe_resume/core/schema.py`) — every extractor returns a list of
  ...
  its module name in `src/vibe_resume/core/runner.py::LOCAL_EXTRACTORS` / `CLOUD_EXTRACTORS` /
...
- **User files are gitignored** — both `profile.yaml` (contains real PII) and
  ...
  `src/vibe_resume/core/config.py::load_config` auto-bootstraps `config.yaml` from the example
