...
2026 年 agent-skills 生态已收敛成**四条安装路径** —— 按你的 agent 选一条,不用再写八条 `ln -s`。

**Tier 0 —— Plugin marketplace(bundled 安装,skill 会被 namespace)**
...
codex
/plugins   # 在 TUI 里找到 vibe-resume 并安装
```

Codex plugin 支持从 [codex-cli v0.121.0](https://developers.openai.com/codex/changelog)(2026-04-15)才开始提供。更旧版本请改走 Tier 1(`npx skills add …`)。你也可以在 `/plugins` 面板交互式添加 marketplace,不一定要用 shell 指令。

Plugin 会把 skill 以及未来要带的 commands / agents / hooks / MCP servers 打包成一个安装单元。skill 以 `/vibe-resume:ai-used-resume`(有 namespace)调用,而不是直接 `/ai-used-resume`。

**Tier 1 —— 37+ 家 `agentskills.io` 标准 host(一行装到所有)**
```bash
...
| 命令 | 功能 |
|---|---|
| `vibe-resume extract [--only NAME]` | 执行 extractor,缓存到 `data/cache/*.json` |
| `vibe-resume aggregate` | 按项目分组、分类任务、推断技术栈 |
| `vibe-resume enrich [-n N] [--locale L] [--tailor JD.txt]` | 生成 summary + achievements(英文用 XYZ,中日德法韩用名词短语);`--tailor` 让 bullet 偏向 JD 关键字 |
| `vibe-resume render -f md\|docx\|pdf\|all [--locale L]` | 渲染 + git 快照 |
| `vibe-resume render --all-locales [-f FMT]` | 一次渲染全部已注册 locale |
| `vibe-resume render --tailor data/imports/jd.txt` | 针对特定 JD 定制 |
| `vibe-resume review [-v N \| --file PATH] [--locale L] [--jd JD.txt]` | 按 8 项 reviewer 清单打分 |
| `vibe-resume trend [--locale L]` | 按 locale 显示历次评分 + 平均 + 最新等级 |
| `vibe-resume completion {bash\|zsh\|fish} [--install]` | 生成或安装 shell 补全脚本 |
| `vibe-resume status` | 显示各来源的活动数 |
| `vibe-resume list-versions` / `vibe-resume diff 1 2` | 简历版本历史 |
...
```bash
uv run vibe-resume render -f md  --locale en_US     # ATS 优化美式默认
uv run vibe-resume render -f md  --locale zh_CN     # 简体中文简历
uv run vibe-resume render -f all --locale ja_JP     # 履歴書 (DOCX 格子) + 職務経歴書 (md/pdf)
uv run vibe-resume render -f md  --locale de_DE     # Lebenslauf 含 Persönliche Daten 区块
```
...
`vibe-resume render --locale zh_CN` 永远胜过 `preferred_locale`;省略 `--locale` 则由 `preferred_locale` 接管。`enrich` 调 LLM 时也走同一条链,确保语言标签注入正确。
...
uv run vibe-resume render --all-locales -f docx         # 强制特定格式
uv run vibe-resume render --all-locales --tailor jd.txt # 一份 JD 用到所有 locale
```
...
能过 Tech Lead 关的简历,在 HR 初筛可能直接被过滤,反之亦然。`--persona` 加入第 3 个轴(locale 是语言、`--tailor` 是 JD,persona 是**预期读者角色**),改的是 bullet 的语气、重点、术语密度 —— 同样的活动数据、同样的 locale,换个读者就重新发声。

| Key | 读者 | 这种读者在扫什么 |
|---|---|---|
| `tech_lead` | Staff+ 工程师 / 技术主管 | 具名系统、具体性能数字、权衡类动词(*migrated / replaced / introduced*) |
| `hr` | HR / 招聘专员 | 职业轨迹、跨职能协作、平易近人的业务影响;满版缩写会被跳过 |
| `executive` | VP / 业务决策者 | 业务产出(营收、成本、团队规模、市场覆盖);每段 role 的首 bullet 要像「标题」 |
| `startup_founder` | 创业团队(A 轮前) | 端到端 ownership、出货速度、资源意识;企业流程式描述会被打折 |
| `academic` | 学术 / 研究评审委员 | 方法论严谨性、数据集、benchmark、citation 式框架 |
...
## Reviewer-view 审核(`vibe-resume review`)
...
uv run vibe-resume review -v 12 --jd jd.txt  # 加入 JD 关键字覆盖率
```
...
彻底搜过 2026 的生态 —— **没有直接对手**同时做到
(1) 多来源 AI 工具提取、(2) 10 locale 渲染、(3) reviewer 审核。最接近的几家只覆盖其中一项:

- [whoisjayd/gitresume](https://github.com/whoisjayd/gitresume) —— 最接近的同领域工具;从本地 / remote GitHub repo 提取 → 简历 bullet。单一来源(仅 git)、单语。`vibe-resume` 多了 18 个 AI 工具 extractor、10 locale、reviewer 审核。
- [AmirhosseinOlyaei/AI-Resume-Builder](https://github.com/AmirhosseinOlyaei/AI-Resume-Builder) —— 从 PR 描述和 commit message 生成 bullet。同样的 thesis,但来源更窄(没有 Claude Code / Cursor / ChatGPT)。
- [AndreaCadonna/resumake-mcp](https://github.com/AndreaCadonna/resumake-mcp) —— 通过 MCP 生成 LaTeX 简历;只做 render,没有 extraction 层。互补于我们 pipeline 的 render 阶段。
- [javiera-vasquez/claude-code-job-tailor](https://github.com/javiera-vasquez/claude-code-job-tailor) —— 在 Claude Code 内把 YAML 简历定制成 JD-tailored PDF。互补于 JD 匹配阶段;我们的 `--tailor` 概念相同,但从提取出的活动开始,而非手动维护的 YAML。

`vibe-resume` 是此列表中唯一同时做到**多 AI 工具提取 + 10 个文化专属 locale 渲染 + reviewer 审核含趋势追踪**的工具。
