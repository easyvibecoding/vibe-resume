...
| **隱私** | 全本地;預設模式 LLM 工作留在當前 Claude Code session(吃訂閱額度);`--mode subprocess` 才走 `claude -p`(2026-06-15 後吃 Agent SDK 額度池)。 | 視情況(OpenAI key 可選) | 必須雲端 API | 全本地 |
...
Slug 以 2026-04-22 的 `skills@latest` 實測為準。跑 `npx skills add … -a unknown` 會印出最新清單。
...
Universal fallback:`-a universal` 會把 skill 放到 `.agents/skills/`,Claude Code / Codex / Gemini CLI / Cursor / Warp / Amp / Cline / OpenCode / Antigravity 原生就會讀這個路徑 —— 一次裝,全部吃得到。最新 slug 清單請查 [vercel-labs/skills](https://github.com/vercel-labs/skills)。
</details>
...
uv run vibe-resume enrich           # 預設 emit prompts 給 Claude Code session 處理(XYZ bullet)
...
│   ├── enricher.py        # mode dispatcher:prompt(預設)/ subprocess / rule-based
...
├── skills/ai-used-resume/                   # 全 8 家 host 的 canonical skill(透過 symlink)
│   ├── SKILL.md                             # 5-section body,agentskills.io 相容
...
├── .claude-plugin/plugin.json               # Claude Code plugin 清單(marketplace 分發)
└── .codex-plugin/plugin.json                # OpenAI Codex plugin 清單(marketplace 分發)
```
