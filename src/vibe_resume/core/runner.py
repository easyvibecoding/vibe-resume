...
def _load(kind: str, name: str):
    if kind == "local":
        return importlib.import_module(f"vibe_resume.extractors.local.{name}")
    if kind == "cloud":
        return importlib.import_module(f"vibe_resume.extractors.cloud_export.{name}")
    if kind == "aigc":
        return importlib.import_module(f"vibe_resume.extractors.api.{name}")
...
def run_aggregator(cfg: dict[str, Any]) -> None:
    from vibe_resume.core.aggregator import aggregate_from_cache
...
def run_enricher(
    ...
    *,
    ...
    enrich_groups(
        ...
        mode=mode, ingest=ingest, ingest_all=ingest_all,
